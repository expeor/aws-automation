...
타임아웃 + 연결 풀이 설정된 boto3 client를 생성합니다.
botocore 자체 재시도는 비활성화하고, 앱 레벨 재시도(executor)만 사용합니다.
...
# 기본 retry 설정 — botocore 자체 재시도 비활성화 (앱 레벨 retry만 사용)
DEFAULT_MAX_ATTEMPTS = 1
DEFAULT_RETRY_MODE: RetryMode = "standard"
...
def get_client(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        max_attempts: 최대 시도 횟수 (기본: 1, botocore 재시도 비활성화)
        retry_mode: 재시도 모드 (기본: 'standard')
