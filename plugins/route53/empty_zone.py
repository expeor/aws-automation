...
def generate_report(results: List[Route53AnalysisResult], output_dir: str) -> str:
    ...
    detail_headers = ["Account", "Zone ID", "Domain", "Type", "레코드수", "상태", "Comment", "권장 조치"]
    ...
    for r in results:
        for f in r.findings:
            if f.status != ZoneStatus.NORMAL:
                ...
                ws_detail.cell(row=detail_row, column=4, value="Private" if zone.is_private else "Public")
    ...
    with SessionIterator(ctx) as sessions:
        for session, identifier, region in sessions:
            try:
                ...
                if not zones:
                    console.print("[dim](Zone 없음)[/dim]")
                    continue

                result = analyze_hosted_zones(zones, account_id, account_name)
                results.append(result)

                unused_count = result.empty_zones + result.ns_soa_only_zones
                if unused_count > 0:
                    console.print(f"Zones {result.total_zones}개 / [yellow]미사용 {unused_count}개[/yellow]")
                else:
                    console.print(f"[green]{result.total_zones}개[/green]")
