...
def _analyze_runtime_eol(func: LambdaFunctionInfo, result: LambdaComprehensiveResult):
    ...
    if status == EOLStatus.DEPRECATED:
        ...
        result.issues.append(LambdaIssue(
...
def analyze_comprehensive(
    ...
    for func in functions:
        ...
        for issue in comp_result.issues:
            ...
            elif issue.issue_type in (IssueType.MEMORY_OVERSIZED, IssueType.MEMORY_UNDERSIZED):
...
def generate_report(results: List[ComprehensiveAnalysisResult], output_dir: str) -> str:
    ...
    headers = ["Account", "Region", "전체 함수", "이슈 함수", "런타임 EOL", "메모리 이슈", "에러 이슈", "월 비용", "절감 가능"]
    row = 4
    ...
    issue_headers = [
        "Account", "Region", "Function", "Runtime", "Memory", "이슈 유형", "심각도", "설명", "권장 조치", "절감 가능"
    ]
    ...
    for r in results:
        for comp in r.results:
            for issue in comp.issues:
                ...
                ws_issues.cell(row=issue_row, column=10, value=f"${issue.potential_savings:,.2f}" if issue.potential_savings > 0 else "-")
    ...
    all_headers = [
        "Account", "Region", "Function", "Runtime", "Memory", "Timeout",
        "Invocations (30d)", "Avg Duration", "Errors", "Throttles", "이슈 수", "월 비용"
    ]
    ...
    for r in results:
        for comp in r.results:
            ...
            ws_all.cell(row=all_row, column=8, value=f"{metrics.duration_avg_ms:.1f}ms" if metrics else "-")
    ...
    with SessionIterator(ctx) as sessions:
        for session, identifier, region in sessions:
            try:
                ...
                result = analyze_comprehensive(functions, account_id, account_name, region)
                results.append(result)

                if result.functions_with_issues > 0:
                    console.print(
                        f"함수 {result.total_functions}개 / "
                        f"[yellow]이슈 {result.functions_with_issues}개[/yellow] / "
                        f"${result.total_monthly_cost:,.2f}/월"
                    )
                else:
                    console.print(f"[green]{result.total_functions}개[/green] / ${result.total_monthly_cost:,.2f}/월")
