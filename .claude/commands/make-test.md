...
class TestCollect{Resource}:
    ...
    def test_collect_normal(self, mock_context, mock_boto3_session):
        """정상 수집 케이스"""
        # Arrange
        from functions.analyzers.{service}.{module} import collect_{resource}
...
class TestAnalyze{Resource}:
    ...
    def test_analyze_normal(self):
        """정상 분석 케이스"""
        from functions.analyzers.{service}.{module} import analyze_{resource}
...
class TestRun:
    ...
    def test_run_success(self, mock_reports, mock_parallel, mock_context):
        """정상 실행"""
        from functions.analyzers.{service}.{module} import run
