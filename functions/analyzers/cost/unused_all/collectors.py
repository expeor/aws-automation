"""
functions/analyzers/cost/unused_all/collectors.py - 개별 리소스 수집/분석 함수

각 AWS 리소스 타입별 수집 및 분석 함수를 정의합니다.

모든 collect_* 함수는 동일한 시그니처를 따릅니다:
    - 리전별: ``(session, account_id, account_name, region) -> dict``
    - 글로벌: ``(session, account_id, account_name) -> dict``

반환 딕셔너리 형식:
    - 성공: ``{"total": int, "unused": int, "waste": float, "result": AnalysisResult}``
    - 실패: ``{"error": str}``
"""
...
from core.shared.aws.lambda_.collector import collect_functions_with_metrics
from functions.analyzers.acm.unused import (
    analyze_certificates as analyze_acm_certificates,
)
from functions.analyzers.acm.unused import (
    collect_certificates as collect_acm_certificates,
)
from functions.analyzers.apigateway.unused import (
    analyze_apis as analyze_apigateway_apis,
)
from functions.analyzers.apigateway.unused import (
    collect_apis as collect_apigateway_apis,
)
from functions.analyzers.cloudwatch.alarm_orphan import (
...
from functions.analyzers.cloudwatch.loggroup_audit import (
    analyze_log_groups,
    collect_log_groups,
)
from functions.analyzers.codecommit.unused import (
    analyze_repos as analyze_codecommit_repos,
)
from functions.analyzers.codecommit.unused import (
    collect_repos as collect_codecommit_repos,
)
from functions.analyzers.dynamodb.unused import (
    analyze_tables as analyze_dynamodb_tables,
)
from functions.analyzers.dynamodb.unused import (
    collect_dynamodb_tables,
)
from functions.analyzers.ec2.ami_audit import (
    analyze_amis,
    collect_amis,
    get_used_ami_ids,
)
from functions.analyzers.ec2.ebs_audit import analyze_ebs, collect_ebs
from functions.analyzers.ec2.eip_audit import analyze_eips, collect_eips
from functions.analyzers.ec2.snapshot_audit import (
    analyze_snapshots,
    collect_snapshots,
    get_ami_snapshot_mapping,
)
from functions.analyzers.ec2.unused import (
    analyze_instances as analyze_ec2_instances,
)
from functions.analyzers.ec2.unused import (
    collect_ec2_instances,
)
from functions.analyzers.ecr.unused import analyze_ecr_repos, collect_ecr_repos
from functions.analyzers.efs.unused import (
...
from functions.analyzers.elasticache.unused import (
    analyze_clusters as analyze_elasticache_clusters,
)
from functions.analyzers.elasticache.unused import (
    collect_elasticache_clusters,
)
from functions.analyzers.elb.target_group_audit import (
    analyze_target_groups,
    collect_target_groups,
)
from functions.analyzers.elb.unused import (
    analyze_load_balancers,
    collect_classic_load_balancers,
    collect_v2_load_balancers,
)
from functions.analyzers.eventbridge.unused import (
    analyze_rules as analyze_eventbridge_rules,
)
from functions.analyzers.eventbridge.unused import (
    collect_rules as collect_eventbridge_rules,
)
from functions.analyzers.fn.unused import analyze_functions as analyze_lambda_functions
from functions.analyzers.fsx.unused import (
    analyze_filesystems as analyze_fsx_filesystems,
)
from functions.analyzers.fsx.unused import (
    collect_filesystems as collect_fsx_filesystems,
)
from functions.analyzers.glue.unused import (
    analyze_jobs as analyze_glue_jobs,
)
from functions.analyzers.glue.unused import (
    collect_glue_jobs,
)
from functions.analyzers.kinesis.unused import (
    analyze_streams as analyze_kinesis_streams,
)
from functions.analyzers.kinesis.unused import (
    collect_kinesis_streams,
)
from functions.analyzers.kms.unused import analyze_kms_keys, collect_kms_keys
from functions.analyzers.opensearch.unused import (
    analyze_domains as analyze_opensearch_domains,
)
from functions.analyzers.opensearch.unused import (
    collect_opensearch_domains,
)
from functions.analyzers.rds.snapshot_audit import (
    analyze_rds_snapshots,
    collect_rds_snapshots,
)
from functions.analyzers.rds.unused import (
    analyze_instances as analyze_rds_instances,
)
from functions.analyzers.rds.unused import (
    collect_rds_instances,
)
from functions.analyzers.redshift.unused import (
...
from functions.analyzers.s3.empty_bucket import analyze_buckets, collect_buckets
from functions.analyzers.sagemaker.unused import (
    analyze_endpoints as analyze_sagemaker_endpoints,
)
from functions.analyzers.sagemaker.unused import (
    collect_sagemaker_endpoints,
)
from functions.analyzers.secretsmanager.unused import (
    analyze_secrets,
    collect_secrets,
)
from functions.analyzers.sns.unused import (
    analyze_topics as analyze_sns_topics,
)
from functions.analyzers.sns.unused import (
    collect_sns_topics,
)
from functions.analyzers.sqs.unused import (
...
from functions.analyzers.vpc.endpoint_audit import (
    analyze_endpoints,
    collect_endpoints,
)
from functions.analyzers.vpc.eni_audit import analyze_enis, collect_enis
from functions.analyzers.vpc.nat_audit_analysis import NATAnalyzer, NATCollector
from functions.analyzers.vpc.sg_audit_analysis import SGAnalyzer, SGCollector, SGStatus
...
def collect_nat(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    ...
    Returns:
        total, unused, waste, findings 키를 포함하는 딕셔너리.
        오류 시 error 키를 포함하는 딕셔너리.
    """
    try:
...
def collect_eni(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    ...
    Returns:
        total, unused, result 키를 포함하는 딕셔너리.
        오류 시 error 키를 포함하는 딕셔너리.
    """
    try:
...
def collect_ebs_volumes(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    ...
    Returns:
        total, unused, waste, result 키를 포함하는 딕셔너리.
        오류 시 error 키를 포함하는 딕셔너리.
    """
    try:
...
def collect_elb(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """ELB (v2 + Classic) 수집 및 미사용 분석

    ALB/NLB/GLB와 Classic LB를 모두 수집합니다.
...
def collect_snapshot(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """EBS Snapshot 수집 및 미사용 분석

    AMI 매핑을 조회하여 고아/오래된 스냅샷을 식별합니다.
...
def collect_ami(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    ...
    실행 중인 인스턴스가 사용하는 AMI를 제외하여 미사용 AMI를 식별합니다.
...
def collect_rds_snapshot(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    ...
    Returns:
        total, old, waste, result 키를 포함하는 딕셔너리.
        오류 시 error 키를 포함하는 딕셔너리.
    """
    try:
...
def collect_loggroup(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    ...
    Returns:
        total, issue, waste, result 키를 포함하는 딕셔너리.
        오류 시 error 키를 포함하는 딕셔너리.
    """
    try:
...
def collect_target_group(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """Target Group 수집 및 문제 분석

    미연결 또는 타겟이 없는 Target Group을 식별합니다.
    ...
    Returns:
        total, issue, result 키를 포함하는 딕셔너리.
        오류 시 error 키를 포함하는 딕셔너리.
    """
    try:
...
def collect_secret(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """Secrets Manager 시크릿 수집 및 미사용 분석
...
def collect_ecr(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    ...
    빈 리포지토리 및 오래된 이미지가 있는 리포지토리를 식별합니다.
...
def collect_lambda(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """Lambda 함수 수집 및 미사용 분석

    CloudWatch 메트릭을 포함하여 함수를 수집합니다.
...
def collect_elasticache(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """ElastiCache 클러스터 수집 및 미사용/저사용 분석
...
def collect_efs(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """EFS 파일시스템 수집 및 미사용 분석

    마운트 타겟 없음, I/O 없음, 빈 파일시스템을 식별합니다.
...
def collect_eventbridge(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """EventBridge 규칙 수집 및 미사용 분석

    비활성화, 타겟 없음, 미사용 규칙을 식별합니다.
...
def collect_cw_alarm(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """CloudWatch Alarm 수집 및 고아 알람 분석

    고아 알람(대상 리소스 없음)과 무동작 알람을 식별합니다.
    ...
    Returns:
        total, orphan, result 키를 포함하는 딕셔너리.
        오류 시 error 키를 포함하는 딕셔너리.
    """
    try:
...
def collect_sagemaker_endpoint(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """SageMaker Endpoint 수집 및 미사용/저사용 분석
...
def collect_route53(session, account_id: str, account_name: str) -> dict[str, Any]:
    """Route53 Hosted Zone 수집 및 빈 Zone 분석 (글로벌 서비스)
    ...
    Returns:
        total, empty, waste, result 키를 포함하는 딕셔너리.
        오류 시 error 키를 포함하는 딕셔너리.
    """
    try:
...
def collect_s3(session, account_id: str, account_name: str) -> dict[str, Any]:
    ...
    Returns:
        total, empty, result 키를 포함하는 딕셔너리.
        오류 시 error 키를 포함하는 딕셔너리.
    """
    try:
...
def collect_redshift(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """Redshift 클러스터 수집 및 미사용/저사용/일시 중지 분석
...
def collect_opensearch(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """OpenSearch 도메인 수집 및 미사용/저사용 분석
...
def collect_kinesis(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """Kinesis Data Streams 수집 및 미사용/저사용 분석
...
def collect_transfer(session, account_id: str, account_name: str, region: str) -> dict[str, Any]:
    """Transfer Family 서버 수집 및 미사용/유휴 분석
