"""
functions/analyzers/elb/inventory.py - ELB 인벤토리 조회 (스트리밍 방식)
...
class ResourceDef:
    ...
    Attributes:
        name: 리소스 표시 이름 (콘솔 출력용)
        sheet_name: Excel 시트 이름
        method: InventoryCollector의 수집 메서드 이름
        method_kwargs: 수집 메서드에 전달할 키워드 인자
        columns: Excel 컬럼 정의 리스트
        row_mapper: 수집 항목을 Excel 행 데이터로 변환하는 함수
    """
...
class Stats:
    ...
    Attributes:
        counts: 리소스 유형별 수집 수량
        type_counts: LB 유형별 (ALB/NLB/GWLB/CLB) 수량
        warnings: 경고 메시지 리스트
    """
...
def run(ctx: ExecutionContext) -> None:
    ...
    Load Balancer와 Target Group을 순차적으로 수집하고,
    수집 즉시 Excel에 기록 후 메모리에서 해제합니다.
