"""
functions/analyzers/elb/alb_unused.py - ALB 미사용 분석

타겟이 없거나 비정상인 Application Load Balancer 탐지
...
def _collect_and_analyze(session, account_id: str, account_name: str, region: str) -> LBAnalysisResult | None:
    """단일 계정/리전의 ALB 수집 및 분석 (parallel_collect 콜백)
...
def run(ctx: ExecutionContext) -> None:
    ...
    멀티 계정/리전에서 Application Load Balancer를 병렬 수집하고,
    미사용 ALB를 식별하여 Excel 보고서를 생성합니다.

    Args:
        ctx: CLI 실행 컨텍스트 (인증, 계정/리전 선택, 출력 설정 포함)
    """
    console.print("[bold]ALB 미사용 분석 시작...[/bold]")
    ...
    if not all_results:
        console.print("[yellow]분석할 ALB 없음[/yellow]")
        return
    ...
    console.print(f"\n[bold]전체 ALB: {totals['total']}개[/bold]")
    if totals["unused"] > 0:
    ...
    filepath = generate_unused_report(all_results, output_path, lb_type_name="ALB")
