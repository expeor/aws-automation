...
class TestCollectSGs:
    ...
    @patch("functions.analyzers.vpc.sg_audit.SGCollector")
    def test_no_security_groups(self, mock_collector_cls):
        """Security Group이 없는 경우"""
        from functions.analyzers.vpc.sg_audit import _collect_sgs
...
class TestCreateOutputDirectory:
    ...
    @patch("functions.analyzers.vpc.sg_audit.OutputPath")
    def test_with_profile_name_sso(self, mock_output_path):
        """profile_name이 있는 경우 (SSO)"""
        from functions.analyzers.vpc.sg_audit import _create_output_directory
    ...
    def test_with_profile_name(self, mock_output_path):
        ...
        mock_path_instance.sub.return_value = mock_path_instance
        mock_path_instance.with_date.return_value = mock_path_instance
        mock_path_instance.build.return_value = "/output/my-profile/vpc/security/2024-01-01"
        mock_output_path.return_value = mock_path_instance
    ...
    def test_default_identifier(self, mock_output_path):
        ...
        mock_path_instance.sub.return_value = mock_path_instance
        mock_path_instance.with_date.return_value = mock_path_instance
        mock_path_instance.build.return_value = "/output/default/vpc/security/2024-01-01"
        mock_output_path.return_value = mock_path_instance
...
class TestRun:
    ...
    @patch("functions.analyzers.vpc.sg_audit.open_in_explorer")
    @patch("functions.analyzers.vpc.sg_audit.SGExcelReporter")
    @patch("functions.analyzers.vpc.sg_audit._create_output_directory")
    @patch("functions.analyzers.vpc.sg_audit.SGAnalyzer")
    @patch("functions.analyzers.vpc.sg_audit.parallel_collect")
    @patch("functions.analyzers.vpc.sg_audit.console")
    def test_no_security_groups(
        ...
        """Security Group이 없는 경우"""
        from functions.analyzers.vpc.sg_audit import run
