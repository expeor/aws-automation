...
class TestCollectAndAnalyze:
    ...
    @patch("functions.analyzers.iam.iam_audit.IAMAnalyzer")
    @patch("functions.analyzers.iam.iam_audit.IAMCollector")
    def test_collect_and_analyze(self, mock_collector_cls, mock_analyzer_cls):
        """정상적인 수집 및 분석"""
        from functions.analyzers.iam.iam_audit import _collect_and_analyze
...
class TestPrintSummary:
    ...
    @patch("functions.analyzers.iam.iam_audit.console")
    def test_empty_stats_list(self, mock_console):
        """빈 통계 리스트"""
        from functions.analyzers.iam.iam_audit import _print_summary
...
class TestCreateOutputDirectory:
    ...
    @patch("functions.analyzers.iam.iam_audit.OutputPath")
    def test_with_sso_session(self, mock_output_path):
        """SSO 세션이 있는 경우 (프로파일명 사용)"""
        from functions.analyzers.iam.iam_audit import _create_output_directory
...
class TestRun:
    ...
    @patch("functions.analyzers.iam.iam_audit.open_in_explorer")
    @patch("functions.analyzers.iam.iam_audit.IAMExcelReporter")
    @patch("functions.analyzers.iam.iam_audit._create_output_directory")
    @patch("functions.analyzers.iam.iam_audit._print_summary")
    @patch("functions.analyzers.iam.iam_audit.parallel_collect")
    @patch("functions.analyzers.iam.iam_audit.console")
    def test_no_results(
        self,
        mock_console,
        ...
        """결과가 없는 경우"""
        from functions.analyzers.iam.iam_audit import run
