...
class TestGetRdsPrices:
    ...
    def test_returns_cached_prices_without_session(self):
        """세션 없이 캐시된 가격 반환"""
        from core.shared.aws.pricing.rds import get_rds_prices
    ...
    @patch("core.shared.aws.pricing.rds.get_rds_prices_from_api")
...
class TestGetRdsMonthlyCost:
    ...
    def test_single_az_calculation(self):
        """Single-AZ 비용 계산"""
        from core.shared.aws.pricing.rds import HOURS_PER_MONTH, get_rds_monthly_cost
...
class TestGetRdsPricesFromApi:
    ...
    @patch("core.shared.aws.pricing.rds.get_client")
    def test_successful_api_call(self, mock_get_client):
        """API 호출 성공"""
        from core.shared.aws.pricing.rds import get_rds_prices_from_api
