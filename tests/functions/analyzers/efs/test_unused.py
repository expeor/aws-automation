...
def make_efs_info(
    ...
    """EFSInfo 테스트 데이터 생성"""
    from functions.analyzers.efs.unused import EFSInfo
...
class TestCollectEfsFilesystems:
    ...
    def test_collect_normal(self, mock_boto3_session):
        """정상 수집 케이스"""
        from functions.analyzers.efs.unused import collect_efs_filesystems
        ...
        with (
            patch("functions.analyzers.efs.unused.get_client", side_effect=get_client_mock),
            patch("functions.analyzers.efs.unused.batch_get_metrics", return_value={}),
        ):
    ...
    def test_collect_empty(self, mock_boto3_session):
        ...
        with patch("functions.analyzers.efs.unused.get_client", side_effect=get_client_mock):
            result = collect_efs_filesystems(
                mock_boto3_session,
                account_id="123456789012",
                account_name="test-account",
...
class TestAnalyzeFilesystems:
    ...
    def test_analyze_normal(self):
        """정상 파일시스템 분석"""
        from functions.analyzers.efs.unused import FileSystemStatus, analyze_filesystems
    ...
    def test_analyze_empty_input(self):
        """빈 입력 케이스"""
        from functions.analyzers.efs.unused import analyze_filesystems
...
class TestRun:
    ...
    @patch("functions.analyzers.efs.unused.parallel_collect")
    @patch("functions.analyzers.efs.unused.generate_report")
    @patch("functions.analyzers.efs.unused.open_in_explorer")
    def test_run_success(self, mock_explorer, mock_report, mock_parallel, mock_context):
        """정상 실행"""
        from functions.analyzers.efs.unused import EFSAnalysisResult, run
    ...
    def test_run_no_results(self, mock_parallel, mock_context):
        """결과 없음"""
        from functions.analyzers.efs.unused import run
...
class TestGenerateReport:
    ...
    def test_generate_report_creates_file(self, tmp_path):
        """보고서 파일 생성 테스트"""
        from functions.analyzers.efs.unused import EFSAnalysisResult, EFSFinding, FileSystemStatus, generate_report
    ...
    def test_generate_report_empty_results(self, tmp_path):
        """빈 결과 보고서"""
        from functions.analyzers.efs.unused import generate_report
...
class TestCollectAndAnalyze:
    ...
    @patch("functions.analyzers.efs.unused.collect_efs_filesystems")
    @patch("functions.analyzers.efs.unused.analyze_filesystems")
    def test_collect_and_analyze_success(self, mock_analyze, mock_collect, mock_boto3_session):
        """정상 수집 및 분석"""
        from functions.analyzers.efs.unused import EFSAnalysisResult, _collect_and_analyze
    ...
    def test_collect_and_analyze_no_filesystems(self, mock_collect, mock_boto3_session):
        """파일시스템 없음"""
        from functions.analyzers.efs.unused import _collect_and_analyze
