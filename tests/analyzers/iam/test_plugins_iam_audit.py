...
class TestCollectAndAnalyze:
    ...
    @patch("analyzers.iam.iam_audit.IAMAnalyzer")
    @patch("analyzers.iam.iam_audit.IAMCollector")
    def test_collect_and_analyze(self, mock_collector_cls, mock_analyzer_cls):
        """정상적인 수집 및 분석"""
        from analyzers.iam.iam_audit import _collect_and_analyze
...
class TestPrintSummary:
    ...
    @patch("analyzers.iam.iam_audit.console")
    def test_empty_stats_list(self, mock_console):
        """빈 통계 리스트"""
        from analyzers.iam.iam_audit import _print_summary
...
class TestRun:
    ...
    @patch("analyzers.iam.iam_audit.open_in_explorer")
    @patch("analyzers.iam.iam_audit.IAMExcelReporter")
    @patch("analyzers.iam.iam_audit._create_output_directory")
    @patch("analyzers.iam.iam_audit._print_summary")
    @patch("analyzers.iam.iam_audit.parallel_collect")
    @patch("analyzers.iam.iam_audit.console")
    def test_no_results(
        self,
        mock_console,
        ...
        """결과가 없는 경우"""
        from analyzers.iam.iam_audit import run
