...
class TestCollectSGs:
    ...
    @patch("analyzers.vpc.sg_audit.SGCollector")
    def test_no_security_groups(self, mock_collector_cls):
        """Security Group이 없는 경우"""
        from analyzers.vpc.sg_audit import _collect_sgs
...
class TestRun:
    ...
    @patch("analyzers.vpc.sg_audit.open_in_explorer")
    @patch("analyzers.vpc.sg_audit.SGExcelReporter")
    @patch("analyzers.vpc.sg_audit._create_output_directory")
    @patch("analyzers.vpc.sg_audit.SGAnalyzer")
    @patch("analyzers.vpc.sg_audit.parallel_collect")
    @patch("analyzers.vpc.sg_audit.console")
    def test_no_security_groups(
        ...
        """Security Group이 없는 경우"""
        from analyzers.vpc.sg_audit import run
