...
from analyzers.cost.pricing.constants import DEFAULT_PRICES
...
class TestPricingService:
    ...
    def test_get_prices_cache_hit(self, mock_cache, sample_ec2_prices):
        """캐시 히트 시 캐시 데이터 반환"""
        from analyzers.cost.pricing.utils import PricingService
    ...
    def test_metrics_increment(self):
        """메트릭 증가 테스트"""
        from analyzers.cost.pricing.utils import PricingMetrics
...
class TestGetPricesFunction:
    ...
    def test_get_prices_function_exists(self):
        ...
        from analyzers.cost.pricing.utils import get_prices

        assert callable(get_prices)

    def test_pricing_service_singleton(self):
        """pricing_service가 싱글톤인지 확인"""
        from analyzers.cost.pricing.utils import pricing_service
