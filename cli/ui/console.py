...
def print_stat_line(label: str, value: Any, style: str = "white") -> None:
    """통계 한 줄 출력
...
def print_stat_box(title: str, stats: Dict[str, Any]) -> None:
    """통계 박스 출력

    Args:
        title: 박스 제목
        stats: {"label": value, ...} 딕셔너리
    """
    console.print()
    console.print(f"[bold #FF9900]━━━ {title} ━━━[/]")
    for label, value in stats.items():
        if isinstance(value, int):
            style = "cyan" if value > 0 else "dim"
        elif isinstance(value, str) and value.startswith("!"):
            style = "red"
            value = value[1:]
        else:
            style = "white"
        console.print(f"  {label}: [{style}]{value}[/]")
...
def print_result_summary(
    ...
    """
    console.print()
    parts = [f"전체: [cyan]{total}[/]"]
    if found > 0:
        parts.append(f"발견: [yellow]{found}[/]")
    if warning > 0:
        parts.append(f"경고: [yellow]{warning}[/]")
    if error > 0:
        parts.append(f"에러: [red]{error}[/]")
    console.print("  " + "  |  ".join(parts))


def print_no_results(message: str = "결과 없음") -> None:
    """결과 없음 메시지
...
def print_divider(style: str = "dim") -> None:
    """구분선 출력"""
    console.print(f"[{style}]" + "─" * 50 + f"[/{style}]")
